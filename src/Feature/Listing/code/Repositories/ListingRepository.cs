...
namespace EMAAR.ECM.Feature.Listing.Repositories
{
    ...
    public class ListingRepository : IListingRepository
    {
        ...
        public SearchResultsGeneric<ListingSearchResultItem> GetListingModel(int pageNumber = -1, int pageSize = -1, string filter = "", string itemId = "", string listItemTemplateId = "", bool showFilters = false)
        {
            ...
            conditions.Add(new SearchCondition() { Name = CommonConstants.TemplateID, Value = listItemTemplateId, CompareType = CompareType.ExactMatch });
            conditions.Add(new SearchCondition() { Name = Sitecore.ContentSearch.BuiltinFields.Path, Value = SearchHelper.FormatGuid(id.ToString()), CompareType = CompareType.ExactMatch });
